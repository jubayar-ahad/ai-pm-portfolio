...
Skills in nested project directories are automatically scoped to files inside that directory. Skills in the repo-wide `.cursor/skills/` are available everywhere. This is similar to the `paths` frontmatter field — you don't need to set `paths` on a nested skill to scope it to its directory.
